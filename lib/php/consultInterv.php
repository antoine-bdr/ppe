...
if ($_SESSION['idConnexion']!=0 && $_SESSION['id_droit']==1 || $_SESSION['id_droit']==3){
  //Si l'utilisateur c'est connecté et qu'il a un id equivalent à celui d'un administrateur ou d'un assistant alors il peut accéder au site
    $connect = mysqli_connect("localhost", "root", "", "e4"); //Connexion à la BDD
    $query = "SELECT * FROM intervention, agence, client, employe, technicien WHERE intervention.numClient = client.numClient AND agence.numAgence = technicien.numAgence AND intervention.matricule = employe.matricule AND technicien.matricule = employe.matricule ORDER BY NumIntervention"; //On selectionne les éléments de la table intervention, agence, client, employe, technicien
...
<body class="index">
  ...
  <center><h2> Affichage des interventions </h2></center>
<table class="display" id="example">
  ...
  $i = 1;
    while($row = mysqli_fetch_array($result)){
       //Tant qu'il y a de résultats correspondant à la requête
  ?> 
    <tr class="gradeX"> 
      <td><?php echo $i; ?></td>
      ...
      <td><a href ="modifIntervention.php?id=<?php  echo $row['NumIntervention']; ?>">Modifier</a></td>
    </tr> 
  <?php $i++; } ?>
  </tbody>
