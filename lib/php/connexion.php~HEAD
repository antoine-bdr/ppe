      ...
      if($login&&$mdp)//Si l'utilisateur a entré tout les champs
       {
            ...
            $requser = $bdd->prepare("SELECT * FROM connexion,droits WHERE droits.id_droit= connexion.id_droit AND login = ? AND password = ? ");//On prépare la requête selectionnant la table connexion et droits
